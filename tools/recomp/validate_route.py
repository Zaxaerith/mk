...
def run(exe, rom, directory, frames, intercepts):
    directory.mkdir()
    ...
    return snapshots, int(hits[1]) if hits else 0
...
def main():
    ...
    ref, _ = run(args.exe.resolve(), args.rom.resolve(), directory / 'reference', args.frames, None)
    native, hits = run(args.exe.resolve(), args.rom.resolve(), directory / 'native', args.frames, args.intercepts)
